...
public class HoOnQuery extends HoOnMessage{
    ...
    public static final String TYPE = "QUERY"; //represent the query type 
	 ...
	 * @throws HoOnException if deserialization fails
	 */
	...
	public byte[] encode() throws IOException {
		...
		DataOutputStream query_out = new DataOutputStream(b);
		query_out.writeByte(HoOnMessage.QUERY_HEADER);
		
		int errorCodeValue = ErrorCode.NOERROR.getErrorCodeValue();
		query_out.writeByte(errorCodeValue);
		
		int tmp = (int) (queryId & 0xffffffffL);
		query_out.writeInt(tmp);
		
		short requestedPostsValue = (short) (requestedPosts & 0xffff);
		query_out.writeShort(requestedPostsValue);
