...
public abstract class HoOnMessage {
	...
	public static final byte MASK_VERSION = (byte)240; // the mask for right version
	public static final byte CORRECT_VERSION = 32; // the mask for right version
    ...
    public static boolean checkVersion(byte b){
    	if((b & MASK_VERSION) == CORRECT_VERSION){
    		return true;
    	}
	...
	public boolean isLegalPostsNumber(int requestedPosts) {
		if(requestedPosts > MAX_POST_NUMBER || requestedPosts < MIN_POST_NUMBER){
			return false;
		}
	 ...
	 */
	public static boolean isLegalId(long queryId) {
